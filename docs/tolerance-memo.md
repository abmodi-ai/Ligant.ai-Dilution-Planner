...
With c_src_exact = c_src_nominal·(1 + ε_src), the vessel's exact concentration is c_target·(1 + ε_src)·Π(1 + δᵢ), so |ε| ≤ |ε_src| + (number of roundings)·u to first order. The step from a relative error to a count of ULPs does not rest on discarding second-order terms: two doubles in the same binade differ by an integer number of ULPs, and the bound is stated as that integer count. For any double c, c/ulp(c) < 2⁵³ = 1/u, so a relative error of k·u is fewer than k ULP of c. The largest per-step count is six (diluent-volume basis), so the bound is stated uniformly as 6 ULP per step; a point fed through an intermediate is two steps from its source and is covered by 5 + 4 (or 5 + 6) ≤ 12.
...
**M1 — the reference value.** The round-trip is measured against **the target converted to the engine's internal unit**, not against the target as the user typed it and not against any later re-conversion. Conversion to the internal unit is one multiplication by an integer power of ten (C3-UN-03), applied once on entry; the ULP distance in §1 is computed between the recomputed exact concentration and that internal value, both as doubles in the same unit. Stating it matters because ulp(c) depends on the magnitude, so the same physical departure measured in µg/mL and in mg/mL is a different number of ULPs.

**Evidence** (`scripts/empirical-sample.mjs`, 200,000 random inputs, 142,623 plans, all bases and routes, chains to 10 steps): maximum 3 ULP at 1 step, 5 at 2, 6 at 3, 7 at 4, 9 at 5, 11 at 10. Against the bound 6k these are 50–18% of the bound. The 3 ULP observed at one step is evidence that the bound is not loose; it does not show that the per-step count of roundings is reached, and no sample is claimed to reach it. Independent reimplementation in Python (`verify/reimplementation.py`) agrees with the engine on all 46 vessels of the reference set within the bound and reproduces every displayed value exactly (acceptance 3).
...
**Derivation.** From C3-IV-01, Tᵈ + Dᵈ = V + ρ exactly. The achieved concentration is c_src_ach·Tᵈ/(Tᵈ + Dᵈ) and the target is c_src_nom·T/V (T unrounded), so the ratio is (c_src_ach/c_src_nom)·(Tᵈ/T)·(V/(V + ρ)). |Tᵈ − T| ≤ h_T because Tᵈ is T rounded half away from zero at 3 sf (when the rounding carries into a higher decade the error is smaller still), and T ≥ Tᵈ − h_T, giving the ε_T bound. ρ is the single rounding of the derived volume, |ρ| ≤ h_D (C3-IV-01). The bound is exact in the two magnitudes and makes no first-order approximation; it is evaluated per point from that point's displayed values, so the output states a bound specific to the plan (C3-OUT-03), and the register states the worst-case bound.

**Worst case over leading digit.** Both terms are largest at leading digit 1: h/(x − h) = 0.005/0.995 = 5.03 × 10⁻³ for Tᵈ = 1.00, and h_D/V ≤ 0.005/0.995 for Dᵈ at leading digit 1 in a different decade from Tᵈ. Per step, the **worst-case bound** is 1.005025/0.994975 − 1 = **1.01 × 10⁻²**. It is a bound, not an attained value: both terms sit at leading digit 1 in different decades simultaneously, which no single vessel realises. Where closure is exact, or under the diluent-volume basis: **5.03 × 10⁻³**. Along a chain of n steps: (1.0101)ⁿ − 1, first-order n × 1.01 × 10⁻². At leading digit 9 the per-step worst case is 1.11 × 10⁻³.
